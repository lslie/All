...
class People(object):
    ...
    def make_gun(self,take_gun):
       self.gun=take_gun
       print("%s拿起%s"%(self.name,self.gun))
    #开枪
    ...
    def __str__(self):
        if self.gun:
            ...
            if self.hp>0:
                ...
                return "%s没有枪,被老王打了一枪现在的hp是%d"%(self.name,self.hp)
            else:
...
class Gun(object):
    ...
    # 测试枪里的子弹数量
    def __str__(self):
        return "有%s发子弹的%s" % (self.cilp_item,self.cilp)
'''6.1.3. 子弹类
属性
杀伤力
方法
